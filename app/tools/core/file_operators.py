...
class LocalFileOperator(FileOperator):
    ...
    async def read_file(self, path: PathLike) -> str:
        """Read content from a local file with robust encoding handling."""
        try:
            # Try UTF-8 first
            return Path(path).read_text(encoding="utf-8")
        except UnicodeDecodeError:
            # Fallback to ISO-8859-1 (Latin-1) which accepts any byte sequence
            try:
                ...
                try:
                    ...
                    raise ToolError(f"Failed to read {path}: {str(final_e)}") from None
        except Exception as e:
